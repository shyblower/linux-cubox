/*
 * linux/drivers/video/dovefb/dovefb_base.c -- Marvell DOVE LCD Controller
 *
...
static int proc_lcd_write(struct file *file, const char *buffer,
...
__init int dump_lcd_init_module(int id, struct fb_info *fi)
{
	...
	printk(KERN_INFO "Initialize /proc/mv_lcd%d\n", id);
	printk(KERN_INFO "use cat /proc/mv_lcd%d to see reg settings\n", id);
...
int dovefb_check_var(struct fb_var_screeninfo *var, struct fb_info *fi)
{
	...
	/*
	 * check xres and yres is not 0.
	 */
	if (var->xres == 0 || var->yres == 0) {
		printk(KERN_WARNING "Resolution should not be 0 (%dx%d).\n",
			var->xres, var->yres);
		return -EINVAL;
	}
...
unsigned int dovefb_dump_regs(struct dovefb_info *dfi)
{
	u32 i;
	u32 reg;
	pr_debug("Inside dovefb_lcd_dump:\n");

	for (i = 0xC0; i <= 0x01C4; i += 4) {
		reg = readl(dfi->reg_base + i);
		pr_debug("Dumping LCD: offset=0x%08x, "
			"reg=0x%08x\n", i, reg);
	}
		...
		struct resource *res)
{
	 ...
	 */
	dfli->fb_size = PAGE_ALIGN(DEFAULT_FB_SIZE*2 /* Originally 3 */);
#ifdef USING_SAME_BUFF
...
//#ifdef CONFIG_ARCH_DOVE
	...
	if (type == DOVEFB_GFX_PLANE) {
		info->gfx_plane = dfli;
		dump_lcd_init_module(strstr(fi->fix.id, "1") ? 1 : 0, fi);
	} else
		info->vid_plane = dfli;
...
failed:
	pr_debug("DoveFB: dovefb_init_layer() failed (%d).\n", ret);
...
static int __init dovefb_probe(struct platform_device *pdev)
{
	...
	*/
	...
	info->clk = clk_get(&pdev->dev, dmi->clk_name);
	info->clk_src = dmi->clk_src;
	dev_info(&pdev->dev, "use ref clock %s\n", dmi->clk_name);
